...
class Testing::GoldenFlowBaselineService
  ...
  def generate_baseline_markdown
    ...
    GOLDEN_FLOWS.each do |flow_key, flow_config|
      ...
      report << if performance[:status] == :passed
        "✅ **Performance**: PASSED"
      else
      ...
      report << if reliability[:status] == :passed
        "✅ **Reliability**: PASSED"
      else
  ...
  def create_monitoring_configuration
    ...
    File.write("config/golden_flow_monitoring.yml", config.to_yaml)
  ...
  def create_simple_team
    ...
    Membership.create!(user: user, team: team, role_ids: [Role.admin.id])
    user.update!(current_team: team)
