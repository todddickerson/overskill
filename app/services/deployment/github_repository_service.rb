...
class Deployment::GithubRepositoryService
  ...
  def push_file_structure(file_structure, environment = "production")
    Rails.logger.info "[GitHubRepositoryService] Pushing #{file_structure.size} files to repository as single commit for #{environment}"

    # Use batch commit for all files at once (single workflow trigger)
    result = batch_commit_files(file_structure, environment: environment)
  ...
  def batch_commit_files(file_structure, message: nil, environment: "production")
    return {success: false, error: "No repository created"} unless @app.repository_name

    repo_full_name = "#{@github_org}/#{@app.repository_name}"

    # Generate appropriate commit message
    commit_message = message || generate_app_version_commit_message(file_structure, environment)
  ...
  def generate_app_version_commit_message(file_structure, environment = "production")
    ...
    # Determine environment tag for production deployment
    env_tag = (environment == "production") ? " [production]" : ""

    if key_files.any?
      sample_files = key_files.map { |path| File.basename(path) }.join(", ")
      "feat: Update app with #{file_count} files (#{sample_files})#{env_tag}\n\n🤖 Generated by OverSkill AI App Builder\n✨ Ready for Workers for Platforms deployment"
    else
      "feat: Update app with #{file_count} files#{env_tag}\n\n🤖 Generated by OverSkill AI App Builder\n✨ Ready for Workers for Platforms deployment"
    end
  end
  ...
  def generate_unique_repo_name
    # Use obfuscated_id for privacy + timestamp for uniqueness (fixes GitHub fork conflicts)
    base_name = @app.name.parameterize
    timestamp = Time.current.strftime("%H%M%S%L") # Hours+Minutes+Seconds+Milliseconds
    "#{base_name}-#{@app.obfuscated_id}-#{timestamp}"
  end
