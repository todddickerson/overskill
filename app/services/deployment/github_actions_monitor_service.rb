class Deployment::GithubActionsMonitorService
    ...
    if error_logs.any?
      ...
      if fix_result[:success]
        ...
        if push_result[:success]
          ...
          monitor_deployment_with_retry(max_wait_time: 8.minutes, retry_count: retry_count)
        else
          Rails.logger.error "[GithubActionsMonitor] Failed to push fixes: #{push_result[:error]}"
          handle_retry_strategy(error_logs, fix_result, retry_count, "Push failed: #{push_result[:error]}")
        end
      else
        Rails.logger.warn "[GithubActionsMonitor] Automatic fix failed: #{fix_result[:error]}"
        handle_retry_strategy(error_logs, fix_result, retry_count, fix_result[:error])
      end
    else
      Rails.logger.error "[GithubActionsMonitor] No error logs available for automatic fixing"
      handle_retry_strategy([], {success: false, error: "No error logs available"}, retry_count, "No error logs available")
    end
  end
  ...
  def handle_retry_strategy(error_logs, fix_result, retry_count, error_message)
    ...
    if retry_count < max_retries && should_retry?(error_logs, fix_result, retry_count)
      ...
      if latest_runs.any?
        ...
        handle_deployment_failure(latest_run["id"], retry_count + 1)
      else
        Rails.logger.error "[GithubActionsMonitor] No workflow runs found for retry"
        build_final_failure_response(error_logs, fix_result, retry_count, "No workflow runs found for retry")
      end
    else
      ...
      build_final_failure_response(error_logs, fix_result, retry_count, error_message)
    end
  end
  ...
  def determine_max_retries(error_logs)
    ...
    if auto_fixable_count == total_errors && total_errors <= 3
      3 # High confidence in fixing simple errors
    elsif auto_fixable_count >= (total_errors * 0.7) && total_errors <= 5
      2 # Good confidence in fixing most errors
    elsif auto_fixable_count > 0
