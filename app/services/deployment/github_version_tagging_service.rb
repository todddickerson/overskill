...
class Deployment::GithubVersionTaggingService
  ...
  def generate_tag_name
    ...
    "v#{@app_version.version_number.gsub('v', '')}-#{timestamp}"
  end

  def generate_restoration_version_number
    # Increment patch version with 'restored' suffix
    base_version = @app_version.version_number.gsub('v', '').split('.')
  ...
  def get_tree_sha_for_tag(tag_name)
    ...
    if tag_response.success?
      commit_sha = tag_response.parsed_response['object']['sha']
    else
      commit_sha = tag_sha # Lightweight tag points directly to commit
    end
