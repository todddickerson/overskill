module Ai
  ...
  class AppUpdateOrchestratorStreaming
    ...
    def execute!
      ...
      # Load AI standards
      ai_standards = File.read(Rails.root.join('AI_APP_STANDARDS.md')) rescue ""
    ...
    def generate_whats_next_suggestions
      ...
      selected_suggestions = suggestions.shuffle.first(3)
    ...
    def update_message(content, status = nil)
      ...
      if content.start_with?("✅") || content.start_with?("📝") || content.start_with?("⚡")
        ...
        if lines.any? && (lines.last.start_with?("🔍") || lines.last.start_with?("📋") || lines.last.start_with?("🤔"))
          lines[-1] = content
