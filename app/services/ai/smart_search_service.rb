module Ai
  ...
  class SmartSearchService
    ...
    def search_components(component_name, component_type: :react)
      query = case component_type
      when :react
        "(?:function\\s+#{component_name}|const\\s+#{component_name}\\s*=|class\\s+#{component_name})"
      when :function
        "(?:function\\s+#{component_name}|const\\s+#{component_name}\\s*=.*=>)"
      when :class
        "class\\s+#{component_name}"
      else
        component_name
      end
