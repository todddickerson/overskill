  ...
  class AppBuilderV5
    ...
    def execute!
      ...
      begin
        ...
        handle_error(e)
        nil # CRITICAL: Don't continue to finalize_app_generation after error
      end
    end
    ...
    def execute_simple_claude_flow
      ...
      begin
        ...
        call_ai_with_context(user_message)
        ...
        if e.message.include?("undefined method `[]'")
          ...
          error_msg = if e.message.include?("Worker exceeded resource limits")
            "API Error: Worker exceeded resource limits. The request was too large or complex. Please try a simpler request."
          elsif e.message.include?("anthropic.helicone.ai")
            "API Error: Request failed at proxy layer. Please try again in a moment."
          elsif e.message.include?("502") || e.message.include?("Bad Gateway")
            "API Error: Service temporarily unavailable (502 Bad Gateway). Please try again."
          elsif e.message.include?("503") || e.message.include?("Service Unavailable")
            "API Error: Service temporarily unavailable (503). Please try again."
          else
            # Generic HTML error - don't show raw HTML
            "API Error: Received invalid response from server. Please try again."
          end
        else
    ...
    def finalize_app_generation
      ...
      if app.app_files.count > 0 && @completion_status != :failed
        ...
        if deploy_result[:success]
          ...
          @assistant_message.content = if is_app_update
    ...
    def deploy_app
      ...
      Rails.logger.info "[V5_DEPLOYMENT_DEBUG] App status: #{@app.status}, Files: #{@app.app_files.count}"
      ...
      Rails.logger.info "[V5_DEPLOYMENT_DEBUG] use_fast_preview: #{use_fast_preview}, ENV['FAST_PREVIEW_ENABLED']: #{ENV["FAST_PREVIEW_ENABLED"]}"

      if use_fast_preview
        Rails.logger.info "[V5_FAST_DEPLOY] Using EdgePreviewService for instant preview deployment (2-3s)"
        result = deploy_fast_preview
        Rails.logger.info "[V5_DEPLOYMENT_DEBUG] deploy_fast_preview returned: #{result.inspect}"
      else
        Rails.logger.info "[V5_DEPLOY] Using legacy DeployAppJob pipeline (3-5min)"
        result = deploy_standard
        Rails.logger.info "[V5_DEPLOYMENT_DEBUG] deploy_standard returned: #{result.inspect}"
      end
      result
    end
    ...
    def deploy_fast_preview
      ...
      Rails.logger.info "[V5_DEPLOYMENT_DEBUG] Entering deploy_fast_preview method"

      # Broadcast initial fast deployment progress
      Rails.logger.info "[V5_DEPLOYMENT_DEBUG] Broadcasting initial deployment progress"
      ...
      begin
        ...
        Rails.logger.info "[V5_DEPLOYMENT_DEBUG] Creating EdgePreviewService instance"
        ...
        Rails.logger.info "[V5_DEPLOYMENT_DEBUG] Calling EdgePreviewService.deploy_preview"
        result = service.deploy_preview do |progress|
          # Update progress during deployment
          Rails.logger.info "[V5_DEPLOYMENT_DEBUG] EdgePreview progress: #{progress}"
          broadcast_deployment_progress(
            status: "deploying",
        ...
        Rails.logger.info "[V5_DEPLOYMENT_DEBUG] EdgePreviewService result: #{result.inspect}"
    ...
    def send_import_errors_to_ai(errors)
      ...
      begin
        ...
        call_ai_with_context(error_message)
    ...
    def trigger_deployment_if_ready
      ...
      Rails.logger.info "[V5_DEPLOYMENT_DEBUG] App ID: #{@app.id}, Status: #{@app.status}, Files: #{@app.app_files.count}"
      ...
      if @app.app_files.any?
        ...
        Rails.logger.info "[V5_DEPLOYMENT_DEBUG] Status updated to: #{@app.status}"

        # Trigger deployment job - always use deploy_app for preview
        Rails.logger.info "[V5_DEPLOYMENT_DEBUG] @deploy_preview is: #{@deploy_preview.inspect}"
        Rails.logger.info "[V5_DEPLOYMENT_DEBUG] Calling deploy_app method..."
        deploy_result = deploy_app
        Rails.logger.info "[V5_DEPLOYMENT_DEBUG] deploy_app returned: #{deploy_result.inspect}"
        deploy_result
      else
      ...
      loop do
        ...
        when "tool_use"
          ...
          if dispatched_tools.any?
            ...
            Rails.logger.info "[V5_INCREMENTAL] - Text blocks: #{assistant_content.count { |b| b[:type] == "text" }}"
            Rails.logger.info "[V5_INCREMENTAL] - Tool use blocks: #{assistant_content.count { |b| b[:type] == "tool_use" }}"
    ...
    def execute_tools_synchronously(tool_calls)
      ...
      tool_calls.each_with_index do |tool_call, index|
        ...
        tool_result_block[:content] = (result[:error] || result[:content] || "Tool completed successfully")
    ...
    def process_tool_calls(tool_calls)
      ...
      tool_calls.each_with_index do |tool_call, tool_index|
        ...
        Rails.logger.info "[V5_DEBUG] About to execute tool, streaming_executor: #{begin
          streaming_executor.class.name
        rescue
          "nil"
        end}"
        result = begin
        ...
        updated_tool_calls.reverse_each do |tc|
    ...
    def web_search(args)
      ...
      serpapi_key ||= begin
        Rails.application.credentials.dig(:serpapi_key)
      rescue
        nil
      end
    ...
    def generate_iteration_guidance
      ...
      guidance << case @iteration_count
      when 1..2
        "📋 SETUP PHASE: Focus on project structure and dependencies"
      when 3..5
        "🏗️  BUILD PHASE: Implement core features and components"
      when 6..8
        "🔍 REFINEMENT PHASE: Test, debug, and optimize"
      else
        "🚀 COMPLETION PHASE: Finalize and deploy"
      end
    ...
    def create_app_version_for_generation
      ...
      if previous_version
        current_version = previous_version.version_number.delete("v").split(".").map(&:to_i)
