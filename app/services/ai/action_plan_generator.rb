module Ai
  class ActionPlanGenerator
    ...
    def generate_file_modification(file, implementation_info)
      ...
      modification[:changes] = if content_lower.include?("color") || content_lower.include?("style")
        generate_style_modification(file.content, content_lower)
      elsif content_lower.include?("text") || content_lower.include?("label")
        generate_text_modification(file.content, content_lower)
      elsif content_lower.include?("button") || content_lower.include?("input")
        generate_element_modification(file.content, content_lower)
      else
        generate_general_modification(file.content, content_lower)
      end
    ...
    def modify_color_classes(classes, new_color)
      ...
      classes.split(" ").map do |css_class|
    ...
    def estimate_implementation_time(steps)
      ...
      steps.each do |step|
        base_time += case step[:action]
        when :add_files
          (step[:files]&.count || 1) * 45
        when :modify_files
          (step[:files]&.count || 1) * 20
    ...
    def estimate_plan_complexity(plan)
      ...
      plan[:steps].each do |step|
        complexity_score += case step[:action]
        when :add_files
          (step[:files]&.count || 1) * 3
        when :modify_files
          (step[:files]&.count || 1) * 2
        when :suggest_components
          1
        else
          1
