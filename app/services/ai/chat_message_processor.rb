  ...
  class ChatMessageProcessor
    ...
    def execute_file_modifications(step)
      ...
      step[:files].each do |file_spec|
        ...
        if file
          ...
          if file_spec[:changes][:type] == :line_replace
            result = apply_line_replacement(file, file_spec[:changes])
            changed_files << file.path if result[:success]
          else
            # Direct content replacement for simple changes
            result = apply_content_replacement(file, file_spec[:changes])
            changed_files << file.path if result[:success]
          end
        else
    ...
    def build_file_generation_prompt(file_spec)
      <<~PROMPT
        ...
        Requirements: #{file_spec[:requirements].join(', ') if file_spec[:requirements]}
