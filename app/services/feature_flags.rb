...
class FeatureFlags
  class << self
    ...
    # Check if GitHub workflow deployment should be disabled (WFP-only mode)
    def disable_github_workflow_deployment?(app: nil, user: nil, team: nil)
      # Force enable GitHub deployment if explicitly set
      return false if ENV["FORCE_GITHUB_DEPLOYMENT"] == "true"

      # Force disable GitHub deployment (WFP-only mode)
      return true if ENV["DISABLE_GITHUB_DEPLOYMENT"] == "true"

      # Gradual rollout by percentage (disable GitHub for X% of deploys)
      if ENV["DISABLE_GITHUB_ROLLOUT_PERCENT"].present?
        percentage = ENV["DISABLE_GITHUB_ROLLOUT_PERCENT"].to_i
        return rollout_by_percentage(app&.id || user&.id || team&.id, percentage)
      end

      # Rollout by team (specific teams get WFP-only deployment)
      if team && ENV["WFP_ONLY_TEAM_IDS"].present?
        wfp_only_teams = ENV["WFP_ONLY_TEAM_IDS"].split(",").map(&:to_i)
        return wfp_only_teams.include?(team.id)
      end

      # Default: Use GitHub backup deployment (false = use GitHub)
      # Set to true to disable GitHub deployment by default
      ENV["WFP_ONLY_DEPLOYMENT"] == "true"
    end
