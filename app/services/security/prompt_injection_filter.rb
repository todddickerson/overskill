...
module Security
  class PromptInjectionFilter
    ...
    def sanitize_input(text)
      ...
      sanitized.gsub!(/(.)\1{#{REPETITION_THRESHOLD},}/o, '\1' * 3)
      sanitized.gsub!(/(\b\w+\b)(\s+\1){#{REPETITION_THRESHOLD},}/o, '\1')
    ...
    def has_excessive_repetition?(text)
      ...
      return true if text.match?(/(.)\1{#{REPETITION_THRESHOLD},}/o)

      # Check word repetition
      words = text.split(/\s+/)
      words.group_by(&:downcase).transform_values(&:count)
