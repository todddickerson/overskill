  ...
  class SecurePromptBuilder
    ...
    def self.format_tool_args(args)
      ...
      args.map do |key, value|
        ...
        if key.to_s.include?('path') || key.to_s.include?('file')
          value = sanitize_file_path(value)
        else
          value = sanitize_user_data(value.to_s)
        end
    ...
    def self.sanitize_file_path(path)
      ...
      cleaned = cleaned.start_with?('src/') ? cleaned : "src/#{cleaned}"
