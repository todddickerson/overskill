module VersionControl
  ...
  class GitService
    ...
    def create_branch(branch_name, checkout = true)
      ...
      # Create new branch
      new_branch = @git.branch(branch_name).create
    ...
    def merge(source_branch, message = nil)
      ...
      # Perform merge
      merge_result = @git.merge(source_branch, message || "Merge branch '#{source_branch}' into #{target_branch}")
    ...
    def get_file_diff(file_path)
      @git.diff.path(file_path).to_s rescue ""
    end
