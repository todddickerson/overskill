#!/usr/bin/env ruby
# Simplified fix for App 57 - creates todos table
# Run with: bin/rails runner scripts/fix_app_57_simple.rb

puts "=== Simple Fix for App 57 ==="

app = App.find(57)
puts "App: #{app.name} (ID: #{app.id})"

# Update app files to use correct table name
app_tsx = app.app_files.find_by(path: 'src/App.tsx')
if app_tsx
  content = app_tsx.content
  
  # Fix table name references to use app_57_todos
  content = content.gsub("from('todos')", "from('app_57_todos')")
  content = content.gsub('.from("todos")', '.from("app_57_todos")')
  content = content.gsub(".from('todos')", ".from('app_57_todos')")
  
  app_tsx.update!(content: content)
  puts "✅ Updated App.tsx with correct table name (app_57_todos)"
end

# Also need to create the auth component
puts "\nAdding authentication component..."

# Check if Auth component exists
auth_component = app.app_files.find_by(path: 'src/components/Auth.jsx')
unless auth_component
  auth_content = <<~JSX
  ...
  puts "✅ Created Auth.jsx component"
end

# Update App.tsx to include authentication
puts "\nUpdating App.tsx to require authentication..."

if app_tsx
  # Check if auth is already imported
  unless app_tsx.content.include?("import { Auth }")
    # Update the App component to include auth
    new_content = <<~TSX
...
export default App
    ...
    app_tsx.update!(content: new_content)
...
begin
  ...
  if result[:success]
    ...
    puts "\n🎉 App 57 updated with:"
    puts "  - Correct table name (app_57_todos)"
    puts "  - Authentication component"
    puts "  - User-scoped queries"
    puts "\n🌐 Preview URL: #{result[:preview_url]}"
    puts "\n⚠️  Note: The table 'app_57_todos' still needs to be created in Supabase"
    puts "You can create it manually with this SQL:"
    puts <<~SQL
