#!/usr/bin/env ruby
# Enhance existing app with comprehensive todo functionality
# Run with: bin/rails runner scripts/enhance_existing_app_with_todos.rb

puts "🚀 Enhancing existing app with comprehensive todo functionality..."
puts "=" * 80

# Use existing app 69 that already has working OAuth
app = App.find(69)
puts "✅ Using app: ##{app.id} - #{app.name}"
puts "URL: #{app.preview_url}"

# Set up database schema for user-scoped todos
...
unless existing_tables.include?("todos")
  ...
  if todos_result[:success]
    ...
    app.app_tables.find_or_create_by(name: todos_schema[:name]) do |table|
      table.description = todos_schema[:description]
      table.schema_json = todos_schema[:columns]
    end
  else
    puts "  ❌ Failed to create todos table: #{todos_result[:error]}"
  end
else
  puts "  ✅ Todos table already exists"
end

unless existing_tables.include?("todo_items")
  items_result = supabase_service.create_table(items_schema[:name], items_schema[:columns])
  if items_result[:success]
    puts "  ✅ Created todo_items table"
    app.app_tables.find_or_create_by(name: items_schema[:name]) do |table|
      table.description = items_schema[:description]
      table.schema_json = items_schema[:columns]
    end
  else
    puts "  ❌ Failed to create todo_items table: #{items_result[:error]}"
  end
else
  puts "  ✅ Todo_items table already exists"
end

# Update main app file to include todo routes
puts "\n⚛️ Updating React application..."
...
# Enhanced Dashboard with todo management
...
# TodoList page with comprehensive item management
...
export function TodoList() {
  ...
  const { user } = useAuth()
  const [list, setList] = useState<TodoListData | null>(null)
...
# Update files
files_to_update = [
  { path: 'src/main.tsx', content: main_content },
...
files_to_update.each do |file|
  existing_file = app.app_files.find_by(path: file[:path])
  if existing_file
    existing_file.update!(content: file[:content])
...
if result[:success]
  ...
  puts "  ✅ Real-time database operations"
  puts "  ✅ Responsive design"
  
  puts "\n🧪 Ready for comprehensive testing!"
  puts "  1. Visit: #{result[:preview_url]}/login"
  puts "  2. Sign up or login with OAuth (already working!)"
  puts "  3. Create multiple todo lists"
  puts "  4. Add items with priorities and due dates"
  puts "  5. Mark items as complete/incomplete"
  puts "  6. Delete items and lists"
  puts "  7. Test user isolation (different users see different data)"
  
  puts "\nApp ID: #{app.id} for Playwright testing"
  puts "Database tables: app_#{app.id}_todos, app_#{app.id}_todo_items"
else
  puts "❌ Deployment failed: #{result[:error]}"
end

puts "\n" + "=" * 80
puts "TodoMaster Pro enhancement complete!"
puts "=" * 80