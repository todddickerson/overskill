...
class Deployment::ViteBuilderServiceTest < ActiveSupport::TestCase
  ...
  test "validates worker size limits" do
    assert Deployment::ViteBuilderService::MAX_WORKER_SIZE < 1.megabyte
    assert Deployment::ViteBuilderService::MAX_WORKER_SIZE == 900.kilobytes
  end

  test "build timeout constants are reasonable" do
    assert Deployment::ViteBuilderService::FAST_BUILD_TIMEOUT == 60.seconds
    assert Deployment::ViteBuilderService::PRODUCTION_BUILD_TIMEOUT == 200.seconds
  end
end
