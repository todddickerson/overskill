...
class Api::V1::AppFilesControllerTest < Api::Test
  setup do
    ...
    @app_file = build(:app_file, app: @app)
    @other_app_files = create_list(:app_file, 3)

    @another_app_file = create(:app_file, app: @app)
  ...
  def assert_proper_object_serialization(app_file_data)
    ...
    assert_equal app_file_data["app_id"], app_file.app_id
  end

  test "index" do
    # Fetch and ensure nothing is seriously broken.
    get "/api/v1/apps/#{@app.id}/app_files", params: {access_token: access_token}
  ...
  test "create" do
    ...
    app_file_data = JSON.parse(build(:app_file, app: nil).api_attributes.to_json)
    app_file_data.except!("id", "app_id", "created_at", "updated_at")
    params[:app_file] = app_file_data

    post "/api/v1/apps/#{@app.id}/app_files", params: params
    assert_response :success
    ...
    # Also ensure we can't do that same action as another user.
    post "/api/v1/apps/#{@app.id}/app_files",
  ...
  test "update" do
    ...
    put "/api/v1/app_files/#{@app_file.id}", params: {
      ...
      app_file: {
        ...
        file_type: "Alternative String Value",
        checksum: "Alternative String Value",
    ...
    assert_equal @app_file.file_type, "Alternative String Value"
    assert_equal @app_file.checksum, "Alternative String Value"
