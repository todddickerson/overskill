...
class Api::V1::AppVersionsControllerTest < Api::Test
  setup do
    ...
    @app_version = build(:app_version, app: @app)
    @other_app_versions = create_list(:app_version, 3)

    @another_app_version = create(:app_version, app: @app)
  ...
  def assert_proper_object_serialization(app_version_data)
    ...
    assert_equal_or_nil app_version_data["user"], app_version.user
    ...
    assert_equal app_version_data["app_id"], app_version.app_id
  end

  test "index" do
    # Fetch and ensure nothing is seriously broken.
    get "/api/v1/apps/#{@app.id}/app_versions", params: {access_token: access_token}
  ...
  test "create" do
    ...
    app_version_data = JSON.parse(build(:app_version, app: nil).api_attributes.to_json)
    app_version_data.except!("id", "app_id", "created_at", "updated_at")
    params[:app_version] = app_version_data

    post "/api/v1/apps/#{@app.id}/app_versions", params: params
    assert_response :success
    ...
    # Also ensure we can't do that same action as another user.
    post "/api/v1/apps/#{@app.id}/app_versions",
